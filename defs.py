...
def adwlan(reg,out=True):
  for i in range(reg):mac1=dwlan("rand");sy("dumpsys wifi scan "+("> /dev/null") if out == False);print("\u001b[32m["+str(i)+","+mac1+"] wlan packet send")
def adbt(reg,out=True):
  for i in range(reg):mac1=dbt("rand");sy("dumpsys bluetooth_manager >"+("> /dev/null"))if out == False;print("\u001b[32m["+str(i)+","+mac1+"] bt packet send")
